...
export default function Hooks() {
  const [FacultiesFormatted, setFacultiesFormatted] = useState(
    Faculties.map((fac) => {
      return (
        <ul>
          <li>{fac.FacultyID}</li>
          <li>{fac.FacultyName}</li>
          <li>{fac.FacultyDesignation}</li>
          <li>{fac.FacultyEducationQualification}</li>
          <li>{fac.FacultyExperience}</li>
          <li>{fac.FacultyWorkingSince}</li>
        </ul>
      );
    })
  );
          ...
          <form class="d-flex" role="search">
            <input
              ...
              onChange={(e) => {
                ...
                let temp = Faculties.filter((fac) => {
                  return (
                    fac.FacultyID == e.target.value ||
                    fac.FacultyName.toLowerCase().includes(
                      e.target.value.toLowerCase()
                    ) ||
                    fac.FacultyWorkingSince.toLowerCase().includes(
                      e.target.value.toLowerCase()
                    ) ||
                    fac.FacultyDesignation.toLowerCase().includes(
                      e.target.value.toLowerCase()
                    ) ||
                    fac.FacultyEducationQualification.toLowerCase().includes(
                      e.target.value.toLowerCase()
                    ) ||
                    fac.FacultyExperience.toLowerCase().includes(
                      e.target.value.toLowerCase()
                    )
                  );
                });
                let change = temp.map((fac) => {
                  return (
                    <ul>
                      <li>{fac.FacultyID}</li>
                      <li>{fac.FacultyName}</li>
